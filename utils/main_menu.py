...
def main_menu_func(stat_btn):
    ...
    profile_btn = CircleButton(image=profile_btn_image, **BACK_BTN_CONFIG, command=lambda: profile_btn_func(
        profile_btn, game_title, start_btn, help_btn, stat_btn, main_img_lbl, back_btn_image))
    ...
    if loaded_constants.get("authorization") is True:
        ...
        start_btn = Button(text="Начать", **BTN_CONFIG, command=lambda: authorization_func(
            profile_btn, game_title, start_btn, help_btn, stat_btn, main_img_lbl, back_btn_image))
